...
func _enter_state(old_state, new_state):
	...
	if team_turn != C.TEAM.PLAYER:
		...
		if scored_tiles.size() == 0:
			print(">no better move tiles finalize_turn ", host.data.entity_name)
			finalize_turn()
			return
		else: 
			print(">found scored_tiles for ", host.data.entity_name)
...
func attack_target():
	print("%s.%s[host.node.attack_target()]: -"%[host.data.entity_name,'ai_attack'])
...
func analyze_tile_scores():
	...
	target = find_target()
	print(host.data.entity_name, " is targeting ", target.data.entity_name)
...
func get_tile_value(tile_pos:Vector2i)->int:
	...
	for ability in host.get_abilities():
		...
		for _target in ability.get_valid_targets(tile_pos):
			print(host.data.entity_name, " found valid target at ", _target.map_position, " ", _target.data.entity_name)
...
func finalize_turn():
	Util.sysprint("%s:finalize_turn"%[host.data.entity_name], "start")
	find_threat()
	to_idle = true
	free_debug()
	Util.sysprint("%s:finalize_turn"%[host.data.entity_name], "end")
	host.turn_end.emit()
...
func get_nearest_target():
	var targets = host.get_enemies()
	print(host.data.entity_name, " targets ", targets.map(func (e):return e.data.entity_name))
	
	targets.map(func (e:Node):
		var distance = Util.get_pathfinding_distance(host.data.team,e.map_position,host.map_position, WorldManager.level.grid)
